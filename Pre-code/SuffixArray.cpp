...
int sum[MAXN], cnt[MAXN], sufRank[MAXL][MAXN],lcpArray[MAXN];
char str[MAXN];
...
void buildlcp()
{
    lcpArray[0]=0;
    for(int i=1;i<nnn;i++){
        lcpArray[i]=lcp(suffix[i-1],suffix[i]);
    }
}
...
int main()
{
    ...
    for(int i=0;i<nnn;i++){
        cout<<suffix[i]<<' '<<lcpArray[i]<<endl; // suffix[i] contain starting position of i'th suffix, lcpArray[i] contain lcp with i-1 suffix
    }
